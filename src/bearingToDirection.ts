import { Windrose, CompassPoints } from "./types";
...
export function bearingToDirection(
  ...
  const directionsWithThresholds: DirectionThreshold[] = [];
  ...
  const match: DirectionThreshold | undefined = directionsWithThresholds.find(
    ({
      ...
      if (bearing === 0) {
        return bearing >= 0 && bearing <= threshold;
      } else {
        return bearing > previousThreshold! && bearing <= threshold;
      }
    }
  );
